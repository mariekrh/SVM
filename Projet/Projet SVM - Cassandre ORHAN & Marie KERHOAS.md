...
Via ce graphique, nous comprenons que l'excentricité et la longueur de l'axe majeur sont les principaux facteurs influençant notre modèle tandis que les autres variables ont une plus faible importance. Cela est également visible lorsque l'on étudie la variabilité de l'influence de chaque caractéristique.
...
Cette étude a porté sur la classification des haricots secs des variétés Dermason et Horoz à partir de leurs caractéristiques morphologiques. Nous avons travaillé sur un jeu de données contenant 13 611 grains de 7 variétés différentes, et avons sélectionné ces deux variétés après une analyse exploratoire initiale.
...
Pour la modélisation, nous avons testé sur les données standardisées 7 modèles différents (Linear SVC, SDG Classifier, SVM avec différents kernels, Logistic Regression et Random Forest). Tous ont atteint une précision de 99% après standardisation des données. La validation croisée à 5 folds a identifié le SVM avec kernel RBF comme le modèle le plus stable, avec une accuracy moyenne de 0.9929. L'optimisation des hyperparamètres n'a pas significativement amélioré ces performances. Si la qualité de l'ensemble de nos modèles était globalement très satisfaisante, nous avons toutefois choisi comme meilleur modèle le SVM avec kernel RBF au vu de son accuracy moyenne.

L'interprétation globale de ce modèle a montré que l'excentricité et la longueur de l'axe majeur sont les facteurs les plus importants pour la classification. Les graphiques de dépendance partielle ont révélé une relation négative pour la rondeur et la solidité (favorisant la classe DERMASON) et positive pour l'excentricité, la longueur de l'axe majeur ainsi que de l'axe mineur (favorisant la classe HOROZ). L'étude s'est terminée par une interprétation locale de la prédiction pour un individu choisi au hasard dans notre jeu test. Cela nous a permis de nous rendre compte plus concrètement des caractéristiques de ce haricot à l'origine de sa classification : son excentricité et la longueur de son axe majeur.

Ces résultats confirment que les haricots Horoz sont généralement plus allongés que les Dermason, qui sont plus arrondis, démontrant l'efficacité des techniques de machine learning pour la classification des haricots secs par variétés.
